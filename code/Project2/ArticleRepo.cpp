...
void ArticleRepo::editArticle(ArticleObject^ u) { // a concatener
    ...
    bdd->executeNonQuery("UPDATE Article SET designation = '" + u->getDesignation() + "', prixHT = '" + u->getPrixHT()->Replace(",", ".") + "', TVA = '" + u->getTVA()->Replace(",", ".") + "', quantite_stock = '" + u->getQuantiteStock() + "', seuil_reaprovisionnement = '" + u->getSeuilReapprovisionnement() + "', prix_achat = '" + u->getPrixAchat()->Replace(",", ".") + "' WHERE [id_article] = " + u->getIdArticle());
}
