...
List<ClientObject^>^ ClientRepo::getClients(void) {
    ...
    query += " FROM [Client] LEFT JOIN [POSSEDER] ON [Client].numero_client = [POSSEDER].numero_client LEFT JOIN [Adresse] ON [POSSEDER].id_adresse = [Adresse].id_adresse";
    ...
    for each (DataRow ^ row in ds->Tables[0]->Rows)
    {
        if (!(bool)row["supprime"]) {
            ClientObject^ u;
            if (list_id->Contains((int)row["numero_client"])) { u = list[list_id->IndexOf((int)row["numero_client"])]; }
            else {
            ...
            if (!row->IsNull("supprime1")){
                if (!(bool)row["supprime1"]) {//si l'adresse n'est pas suprime -> le mettre dans le client
